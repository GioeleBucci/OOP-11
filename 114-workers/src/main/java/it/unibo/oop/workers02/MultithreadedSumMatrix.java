...
public class MultithreadedSumMatrix implements SumMatrix {
  ...
  public double sum(final double[][] matrix) {
    return sumAsList(toList(matrix));
  }

  private static List<Double> toList(final double[][] matrix) {
    return Arrays.stream(matrix)
        .flatMap(list -> Arrays.stream(list).boxed())
        .collect(Collectors.toList());
  }

  private long sumAsList(final List<Double> list) {
    ...
    /*
     * Wait for every one of them to finish. This operation is _way_ better done
     * by
     * using barriers and latches, and the whole operation would be better done
     * with
     * futures.
     */
    long sum = 0;
